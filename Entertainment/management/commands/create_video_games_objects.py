...
# import models
from ...models import Categories, Subcategories, Video_Games
...
class Command(BaseCommand):
    ...
    # create an object for the sub-category (e.g.Video_Games) quiz data
    def handle(self, *args: Any, **options):        
        category_id = options['category_id']
        category_name = options['category_name']

        # call the get_specific_json_category function to get the data for the video_games category
        ...
        if json_response:        
            ...
            if questions:            
                ...
                for idx, question in enumerate(questions, start=1):                
                    ...
                    # retrieve the model dynamically using the app label 'Entertainment' and the constructed model name
                    # this allows model fetching by its name without needing to import it directly
                    try:
                        model = apps.get_model(app_label='Entertainment', model_name=model_name)
                    ...
                    if model != None: ##     
                        ...
                        entertainment_category_obj, category_created = Categories.objects.get_or_create(category='Entertainment')
                        
                        # check if the category was created or indicate that it already exists
                        if category_created == True:
                            self.stdout.write(self.style.SUCCESS(f"Created the Entertainment category for Video_Games objects."))

                        else:
                            self.stdout.write(self.style.NOTICE(f"The Entertainment category for Video_Games objects already exists."))

                        # retrieve the subcategory object representing 'Video Games' from the Subcategories model
                        video_games_subcategory_obj, subcategory_created = Subcategories.objects.get_or_create(subcategory='Video Games')

                        # check if the subcategory was created or indicate that it already exists
                        if subcategory_created == True:
                            self.stdout.write(self.style.SUCCESS(f"Created the Video Games subcategory for Video Games objects."))

                        else:
                            self.stdout.write(self.style.NOTICE(f"The Video Games subcategory for Video Games objects already exists."))
                        ...
                        # use get_or_create in the event video_games objects are already in the db
                        ...
                        question_object, _ = model.objects.get_or_create(category = entertainment_category_obj,
                                                               subcategory = video_games_subcategory_obj,
