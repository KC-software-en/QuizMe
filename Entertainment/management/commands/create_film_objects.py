...
# import models
from ...models import Categories, Subcategories, Film
...
class Command(BaseCommand):
    ...
    # create an object for the sub-category (e.g.Film) quiz data
                ...
                for idx, question in enumerate(questions, start=1):                
                    ...
                    if model != None: ##     
                        ...
                        if category_created == True:
                            self.stdout.write(self.style.SUCCESS(f"Created the Entertainment category for Film objects."))

                        else:
                            self.stdout.write(self.style.NOTICE(f"The Entertainment category for Film objects already exists."))

                        # retrieve the subcategory object representing 'Film' from the Subcategories model
                        film_subcategory_obj, subcategory_created = Subcategories.objects.get_or_create(subcategory='Film')

                        # check if the subcategory was created or indicate that it already exists
                        if subcategory_created == True:
                            self.stdout.write(self.style.SUCCESS(f"Created the Film subcategory for Film objects."))

                        else:
                            self.stdout.write(self.style.NOTICE(f"The Film subcategory for Film objects already exists."))
