...
# import models
from ..models import Music, Video_Games
...
# Create your tests here. Use `py manage.py test Entertainment.tests.test_utils` to run tests in cmd
...
@patch('Entertainment.utils.requests.get')
class TestJsonResponse(TestCase):
    ...
    def test_get_json_categories_ok(self, mock_get_request):  
        ...
        mock_response_ok.json.return_value = {
            "trivia_categories": [
                {"id":12, "name":"Entertainment : category1"}, 
                {"id":15, "name":"Entertainment : category2"}
                ]
        }
    ...
    def test_get_specific_json_category_success(self, mock_get_request):
        ...
        mock_ok_response.json.return_value = {
            "results":[
                {"type":"type1",
                 "difficulty":"level",
                 "category":"Entertainment : category1",
                 "question":"question1",
        ...
        # call the function in views.py that internally call the requests.get
        response_ok = utils.get_specific_json_category(quantity=50, category=15)
    ...
    def test_get_specific_json_category_error(self, mock_get_request):
        ...
        # call the function in views.py that internally call the requests.get
        response_error = utils.get_specific_json_category(quantity=50, category=15)
...
class TestCreateSubcategoryObject(TestCase):
    ...
    def test_create_subcategory_object(self):
        with patch('Entertainment.views.create_subcategory_object') as mock_create_subcategory_object:
...
class TestVariousUtils(TestCase):
    ...
    @patch('Entertainment.views.selection')
    ...
    def test_get_category_names_found(self):
        ...
        mock_response = {
                        "trivia_categories": [
                            {
                                "id": 12, # 12 is a selected_category_id in utils.py
                                "name": "Entertainment : category1"
                            },
                            {
                                "id": 15, # 15 is a selected_category_id in utils.py
                                "name": "Entertainment : category2"
                            }
                            ]
                        }
        ...
        self.assertNotEquals(category_names, mock_category_names, msg='Should check that category1 & category2 is in the mock_response.')
    ...
    def test_no_category_name(self):
        ...
        mock_response = {
                        "trivia_categories": [
                            {
                                "id": 1, # 15 is a selected_category_id in utils.py so its excluded here
                                "name": "Entertainment : category1"
                            }                            
                            ]
                        }
...
class TestCategoryObjects(TestCase):
    ...
    def setUp(self):
        ...
        self.mock_questions = [Music.objects.create(                                       
    ...
    # patch to mock the filter method of the Music.objects manager. This is done to ensure that when filter is called in your utils.category_objects function, it returns the specified self.mock_question_selection instead of actually querying the database.         
    # not a literal file path but rather a string representing the import path of the object you want to mock. 
    @patch('Entertainment.utils.Music.objects.filter')        
    def test_category_objects(self, mock_filter):
        ...
        response = utils.category_objects(self.request, 'Music')
    ...
    @patch('Entertainment.utils.globals')
