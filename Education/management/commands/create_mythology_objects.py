...
'''
# the custom command can be called using `python manage.py create_mythology_objects 20 Mythology` to populate Mythology
                ...
                for idx, question in enumerate(questions, start=1):                
                    ...
                    if model != None: ##            
                        ...
                        if category_created == True:
                            self.stdout.write(self.style.SUCCESS(f"Created the Education category for Mythology objects."))

                        else:
                            self.stdout.write(self.style.NOTICE(f"The Education category for Mythology objects already exists."))

                        # retrieve the subcategory object representing 'Mythology' from the Subcategories model
                        mythology_subcategory_obj, subcategory_created = Subcategories.objects.get_or_create(subcategory='Mythology')

                        # check if the subcategory was created or indicate that it already exists
                        if subcategory_created == True:
                            self.stdout.write(self.style.SUCCESS(f"Created the Mythology subcategory for Mythology objects."))

                        else:
                            self.stdout.write(self.style.NOTICE(f"The Mythology subcategory for Mythology objects already exists."))
                        ...
                        self.stdout.write(self.style.SUCCESS(f"Created quiz object {idx} out of {total_questions} for category_id:{category_id}"))
