                ...
                for idx, question in enumerate(questions, start=1):                
                    ...
                    if model != None: ##            
                        ...
                        if category_created == True:
                            self.stdout.write(self.style.SUCCESS(f"Created the Education category for History objects."))

                        else:
                            self.stdout.write(self.style.NOTICE(f"The Education category for History objects already exists."))
                        
                        # retrieve the subcategory object representing 'History' from the Subcategories model
                        history_subcategory_obj, subcategory_created = Subcategories.objects.get_or_create(subcategory='History')
                        
                        # check if the subcategory was created or indicate that it already exists
                        if subcategory_created == True:
                            self.stdout.write(self.style.SUCCESS(f"Created the History subcategory for History objects."))

                        else:
                            self.stdout.write(self.style.NOTICE(f"The History subcategory for History objects already exists."))
