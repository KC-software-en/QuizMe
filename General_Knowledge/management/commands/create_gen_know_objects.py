...
# the custom command can be called with `python manage.py create_gen_know_objects 9 "General Knowledge"` 
...
class Command(BaseCommand):
    ...
    # create an object for the category (e.g.General Knowledge) quiz data
            ...
            if questions:            
                ...
                for idx, question in enumerate(questions, start=1):                
                    ...
                    if model != None: ##            
                        ...
                        gen_know_category_obj, category_created = Categories.objects.get_or_create(category='General Knowledge')

                        # check if the category was created or indicate that it already exists
                        if category_created == True:
                            self.stdout.write(self.style.SUCCESS(f"Created the General Knowledge category for General Knowledge objects."))

                        else:
                            self.stdout.write(self.style.NOTICE(f"The General Knowledge category for General Knowledge objects already exists."))

                        # General Knowledge has no subcategory
                        ...
                        # use get_or_create in the event General Knowledge objects are already in the db
                        # - it ruturns a tuple, use _ to indicate the boolean is ignored 
                        question_object = model.objects.get_or_create(category = gen_know_category_obj,
