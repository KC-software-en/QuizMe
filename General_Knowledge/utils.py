...
# create a varible to store the API url for the General_Knowledge quizzes
# use an f-str to pass in the parameter for quantity
# -> place {} around 50 in the url then replace '50' with 'quantity'
# then place {} around the category id (from quiz_cat.json) for gen_know & replace '9' with 'category'
# return a list
def get_specific_json_category(quantity: int, category: int):
    general_knowledge_url = f"https://opentdb.com/api.php?amount={quantity}&category={category}"
    response = requests.get(general_knowledge_url)
...
'''
Create a function that will create an object for the sub-categories of the General_Knowledge quiz.
The function & its test is called in Django shell so it is commented out.
'''
# create an object for the sub-category (e.g.General Knowledge) quiz data
...
# this will populate the sub-category (e.g. General_Knowledge) table on the admin site with the quiz data

#def create_subcategory_object(category):   
    # call the get_specific_json_category function to get the data for the General_Knowledge category
